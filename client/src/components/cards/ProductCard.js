...
const ProductCard = ({ product }) => {
  ...
  return (
    <>
      ...
      <Card
        cover={
          <img alt="img" src={images && images.length ? images[0].url : laptop} style={{ height: "300px", objectFit: "cover" }} className="p-1"/>
        }
        actions={[
  ...
  {product.quantity < 1 ? <p className="text-danger">Out of stock </p> : "Add to cart"}
        ...
        <Meta title={`Rs.${price}`}/>
        <br/>
        <Meta description={`${description && description.substring(0, 40)}...`}/>
      </Card>
    </>
  );
};
